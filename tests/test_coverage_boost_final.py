...
def test_cloner_additional_branches():
    with patch('clonebox.cloner.libvirt') as mock_libvirt:
        ...
        with patch.object(cloner, "_get_downloads_dir", return_value=Path("/tmp")), \
             patch.object(SelectiveVMCloner, "_ensure_default_base_image") as mock_ensure:
...
def test_cloner_create_vm_branches():
    with patch('clonebox.cloner.libvirt') as mock_libvirt:
        ...
        with patch.object(cloner, "get_images_dir", return_value=Path("/tmp")), \
             patch.object(cloner, "_ensure_default_base_image", return_value=Path("/tmp/base.qcow2")), \
             patch.object(cloner, "_create_cloudinit_iso", return_value=Path("/tmp/init.iso")), \
             patch.object(cloner, "resolve_network_mode", return_value="user"), \
             patch.object(cloner, "_generate_vm_xml", return_value="<xml/>"), \
...
async def test_dashboard_endpoints(monkeypatch):
    from clonebox.dashboard import api_vms, api_containers, api_vms_json, api_containers_json, dashboard as dashboard_view
    ...
    def mock_run(args):
        ...
        if "list" in args:
            ...
            mock_proc.stdout = json.dumps([{"name": "c1", "image": "img1", "status": "up", "ports": "80"}])
        return mock_proc
...
def test_detector_comprehensive_coverage():
    ...
    def mock_run(args, **kwargs):
        ...
        if "systemctl list-units" in cmd:
            return Mock(stdout="docker.service loaded active running\nnginx.service loaded active running", returncode=0)
        if "ps -eo" in cmd:
            return Mock(stdout="1234 100.0 python3\n5678 200.0 node", returncode=0)
        if "docker ps" in cmd:
            return Mock(stdout="container1\timage1\tUp 1 hour\ncontainer2\timage2\tUp 2 hours", returncode=0)
        if "hostnamectl" in cmd:
            return Mock(stdout="Static hostname: test-host\nOperating System: Ubuntu 22.04.3 LTS", returncode=0)
        if "du -sm" in cmd:
...
def test_profiles_additional():
    ...
    with patch("pathlib.Path.exists", return_value=False), \
         patch("clonebox.profiles.pkgutil.get_data", return_value=b"key: pkg"):
